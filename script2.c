// Initialize Station Mode Setup and Scan for Recognised APs based on SSID and Channel
...
#define ESP_WIFI_SSID     "Localization"//"zezewewe"//"TP-Link_D69A"//"SINGTEL-8E9A"//
#define ESP_WIFI_PASS     "HH970326"// "3141592653589"//"17608732"//"leetaojeiy"//
...
// Left / 1
// #define ESP32_IP_ADDR     "192.168.3.200" // MAC: 10:97:bd:d5:5e:38
// #define ESP32_IP_PORT     4220 



// Right / 2
#define ESP32_IP_ADDR     "192.168.3.201" // MAC: 40:91:51:23:16:0c 
#define ESP32_IP_PORT     4230 

#define ESP_STA_CHAR      'R'
...
// static void scanReadFypApsAndDisplay(int8_t arr[AP_ID_INDEX][NUM_READINGS_PER_ITER],float mean_var[2][NUM_READINGS_PER_ITER],int idx) {
...
void app_main(void) {
    ...
    int ESP_STA_ID;
    int CONNECT_DUR; // Staggered connection to prevent connectivity errors
    if (ESP_STA_CHAR=='L') {
        ESP_STA_ID=1;
        CONNECT_DUR=1500; 
    } else {
    ...
    while (1) {
        ...
        // a - Setup Wi-Fi
        // printf("\nWi-Fi: Connect:\n");
        // vTaskDelay(1000/portTICK_PERIOD_MS); // 5 second delay for initial start-up // 31 Aug reduced to 0 second
        wifiConnect();
        vTaskDelay(CONNECT_DUR/portTICK_PERIOD_MS); // 2.5 second delay to allow for connection // 31 Aug reduced to 1 second
        printf("\nWi-Fi: 1 seconds given, Wi-Fi initiated");
        ...
        while (1){
            if (start_up_sync==1){
                sprintf(&strArr[0],"%c",(char) (ESP_STA_CHAR));
                start_up_sync=0;
            } 
            ...
            // vTaskDelay(100000/portTICK_PERIOD_MS); // 31 Aug Commented out
            wifiDisconnect(); 
            vTaskDelay(2500/portTICK_PERIOD_MS); // 1 second given to disconnect // 31 Aug reduced to 1 second
            wifiConnect(); // 
            vTaskDelay(CONNECT_DUR/portTICK_PERIOD_MS); // 2.5 second given to connect again // 31 Aug reduced to 1 second
        }
        ...
        while (scanAgain) {
            ...
            int len = recvfrom(sock, rx_buffer, sizeof(rx_buffer) - 1, 0, (struct sockaddr *)&source_addr, &socklen); //If no messages are available at the socket, the recvfrom() call waits for a message to arrive unless the socket is nonblocking
            ...
            if (len < 0) {
                ...
                // printf("Received %d bytes from %s:%d ", len, LAPTOP_IP_ADDR, LAPTOP_IP_PORT);
                // printf("[%s]\n", rx_buffer);
                if (strncmp(rx_buffer, "SCAN", 4) == 0) {
                    // printf("Received expected message, time to rescan.\n");
                    scanAgain = false; // exit while loop and rescan
                }
            }
        ...
        printf("\nWi-Fi: Disconnect");
        wifiDisconnect(); 
        // vTaskDelay(1000/portTICK_PERIOD_MS); // 1 second given to disconnect // 31 Aug reduced to 0 second
